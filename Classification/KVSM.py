...
KVSM = SVC(kernel = 'rbf', random_state = 0)
KVSM = KVSM.fit(X_train,Y_train)
y_dash = KVSM.predict(X_test)
accuracy_score(Y_test, y_dash)

accuracy_all.append(accuracy_score(y_dash, Y_test))


y_pred_test = KVSM.predict(X_test)
y_pred_train = KVSM.predict(X_train)

print("Kernel SVM Accuracy: {0:.2%}".format(accuracy_score(y_dash, Y_test)))
...
plt.plot(fpr, tpr, marker='.', label='KSVM')
